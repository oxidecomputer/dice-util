...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    dice_cert_tmpl::write_range(&mut out, "SERIAL_NUMBER", start, end)?;
    ...
    dice_cert_tmpl::write_range(&mut out, "SUBJECT_SN", start, end)?;
    cert.clear_range(start, end);
    ...
    writeln!(out, "pub const CERT_TMPL: [u8; {}] = {};", cert.len(), cert)?;
