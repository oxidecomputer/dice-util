...
impl Attest for AttestSledAgent {
    fn get_measurement_log(&self) -> Result<Log, AttestError> {
        ...
        let measurments = self
            .rt
            .block_on(self.client.rot_measurement_log())?
            .into_inner();
    ...
    fn get_certificates(&self) -> Result<PkiPath, AttestError> {
        let certs = self
            .rt
            .block_on(self.client.rot_certificate_chain())?
            .into_inner();
        Ok(certs
            .0
            .into_iter()
    ...
    fn attest(&self, nonce: &Nonce) -> Result<Attestation, AttestError> {
        let attestation = self
            .rt
            .block_on(self.client.rot_attest(&SledAgentTypes::Nonce(
                SledAgentTypes::Sha3_256Digest(nonce.0),
            )))?
            .into_inner();
